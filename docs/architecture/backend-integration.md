...
## CMS Data Fetching for Multi-Page Routes (Epic 24)

Since Epic 24, the `app/[lang]/` routing layer fetches CMS data via dedicated server-side functions that feed into each page's Server Components:

```typescript
// Core CMS fetch functions (server-side only)
getHotelFull(hotelId: string, lang: string)    // Full hotel data for homepage and shared layout
getRoomsPageData(hotelId: string, lang: string) // Room listings for app/[lang]/rooms
getContactPageData(hotelId: string, lang: string) // Contact info for app/[lang]/contact
getGalleryPageData(hotelId: string, lang: string) // Gallery images for app/[lang]/gallery
getAboutPageData(hotelId: string, lang: string)   // About content for app/[lang]/about
getFaqPageData(hotelId: string, lang: string)      // FAQ content for app/[lang]/faq
```

Each `app/[lang]/<page>/page.tsx` route calls the appropriate function at the Server Component level, passing resolved data as props to sections. The `[lang]` parameter enables i18n-aware data fetching. Old routes under `app/(site)/` now serve as redirect stubs pointing to the new `app/[lang]/` paths.
