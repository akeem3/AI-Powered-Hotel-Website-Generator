...
#### 1. Generation Engine
- **Technology:** LangGraph + LLM Provider (OpenRouter/Anthropic)
- **Purpose:** Orchestrate multi-agent website generation
- **Cost Target:** <$2.00 per generation
- **Performance:** <30 minutes end-to-end

#### 2. Content Layer
- **Storage:** BackBlaze B2 (content + translations)
- **CMS:** Directus (read-only access for runtime)
- **Translation:** DeepL API integration (Epic 13)
...
interface WorkflowState {
  ...
  // Graceful Degradation (Story 13.3.1)
  usedFallback?: Partial<Record<'componentSelector' | 'stylingAgent' | 'contentGenerator' | 'assemblyAgent', boolean>>;
...
abstract class BaseAgent {
  // Core execution method with lifecycle hooks and observability
  ...
  protected async afterExecute(state: WorkflowState, result: Partial<WorkflowState>): Promise<void>;
...
The system uses `LLMProviderFactory` to manage LLM interactions, defaulting to `OpenRouterClient` for cost-effective generation (Kimi K2) but supporting `AnthropicClient` for specialized tasks requiring validation retries.

- **Primary Provider:** OpenRouter (accessing Kimi K2, Claude 3 Haiku, GPT-4o Mini)
- **Fallback Logic:** Automatic fallback between models based on budget and errors
- **Observability:** Integrated with LangFuse for trace-level monitoring
...
interface CostAllocation {
  ...
  breakdown: {
    ...
    buffer: 1.885;            // ~94% safety margin
  };
  
  alerts: {
    warningThreshold: 1.60;   // 80% of budget
    stopThreshold: 1.95;      // 97.5% of budget
  };
}
```
...
1. **LLM Agent Implementation**
   - All 5 agents with Kimi K2 integration
...
- **Performance:** <30 minutes average generation time
