...
- Use secret management services (e.g., Vercel Secrets, GitHub Secrets) for deployment.

### Input Validation
```typescript
// ✅ Good - Validate external input with Zod
import { z } from 'zod';
...
const handleUser = (input: unknown) => {
...
```
- Validate all inputs from external sources (APIs, user forms, URL parameters).
- Sanitize data before using it in dangerous contexts (e.g., `dangerouslySetInnerHTML`).

### Data Protection
- Ensure `DeploymentConfig` and other configuration objects do not leak sensitive environment variables to the client-side bundles unless necessary.
